...
```{r}
acf(channel_growth$`Viewer Growth`, lag.max = 50)
```

```{r}
pacf(channel_growth$`Viewer Growth`, lag.max = 50)
```


```{r}
ts.plot(diff(channel_growth$`Viewer Growth`))

# AR1, MA1
sarima(channel_growth$`Viewer Growth`, 1, 0, 0, P = 0, D = 1, Q = 0, S = 4)
sarima(channel_growth$`Viewer Growth`, 1, 0, 1, P = 0, D = 1, Q = 0, S = 4)
# AR(1) almost identical to ARMA(1, 1), MA adds nothing

# autocorrelation still notable at lag 4
# 5th (3rd) peak disguised by interruption?
sarima(channel_growth$`Viewer Growth`, 1, 0, 0, P = 0, D = 1, Q = 0, S = 5)
```

Barring interruption, time series appears stationary using 
SAIRMA(1, 0, 1) x (0, 1, 0)5
